"""
主旨：使用 Dlib 的 HOG + Linear SVM 模型來即時偵測 WebCam 影像中的人臉

Credits：
改寫自 iT 邦幫忙教學文章 [Day11] Face Detection - 使用OpenCV & Dlib：Dlib HOG + Linear SVM｀
網址：https://ithelp.ithome.com.tw/articles/10264199
作者：山姆大叔
原始碼：https://github.com/saivirtue/face_under_computer_vision/blob/main/face_detection/dlib_hog_svm.py
...
def main():
    ...
    while True:
        ...
        # 所有預測結果
        for rect in rects:
            # 計算 bounding box(邊界框)與準確率 - 取得(左上X，左上Y，右下X，右下Y)的值 (記得轉換回原始frame的大小)
